...
#                 carrier_phase = carrier_phase.to(self.device)

#                 # Convert msg to tensor if it's a list, then convert back to list for forward pass
#                 if isinstance(msg, list):
#                     msg = torch.stack(msg)
#                 msg = msg.to(self.device)

#                 # Convert msg to a list of tensors before passing to the forward function
#                 msg = [msg_i.to(self.device) if isinstance(msg_i, torch.Tensor) else torch.tensor(msg_i).to(self.device) for msg_i in msg]
...
#                 # Additional Metrics
#                 carrier_snr = self.snr(carrier, carrier_reconst).item()
#                 msg_snr = torch.mean(torch.tensor([self.snr(m, r).item() for m, r in zip(msg, msg_reconst)])).item()
#                 accuracy = self.compute_accuracy(msg, msg_reconst)

#                 losses_log['carrier_snr'] = carrier_snr
#                 losses_log['msg_snr'] = msg_snr
#                 losses_log['accuracy'] = accuracy
...
#                 if self.cur_iter % self.print_every == 0:
#                     log = f"[{self.cur_iter}/{len(self.train_loader)}]"
#                     for loss_name, loss_value in losses_log.items():
#                         log += f", {loss_name}: {loss_value:.4f}"
#                     it.set_description(log)
...
#             val_metrics = self.test(data='val')
#             self.log_losses(val_metrics, iteration=epoch)
#             epoch_loss.update(val_metrics)

#             logger.info(f"Epoch {epoch} summary: {epoch_loss}")
...
#             for carrier, carrier_phase, msg, msg_phase in tqdm(data):
#                 # Move msg to the device if it's a list
#                 if isinstance(msg, list):
#                     msg = [m.to(self.device) for m in msg]
#                 else:
#                     msg = msg.to(self.device)
...
#             avg_carrier_snr = np.mean(carrier_snr_list)
#             avg_msg_snr = np.mean(msg_snr_list)
#             logger.info("finished testing!")
#             logger.info(f"carrier loss: {avg_carrier_loss/len(data)}")
#             logger.info(f"carrier SnR: {avg_carrier_snr}")
#             logger.info(f"message loss: {avg_msg_loss/len(data)}")
#             logger.info(f"message SnR: {avg_msg_snr}")
#             logger.info(f"accuracy: {avg_accuracy}")

#             return {
#                 'val_epoch_carrier_loss': avg_carrier_loss / len(data),
#                 'val_epoch_msg_loss': avg_msg_loss / len(data),
#                 'val_epoch_carrier_SNR': avg_carrier_snr,
#                 'val_epoch_msg_SNR': avg_msg_snr,
#                 'val_epoch_accuracy': avg_accuracy,
#             }



#     def compute_accuracy(self, original, reconstructed, threshold=0.1):
#         # Convert original and reconstructed to tensors and move them to the device
#         if isinstance(original, list):
#             original = torch.stack(original).to(self.device)
#         else:
#             original = original.to(self.device)
        
#         if isinstance(reconstructed, list):
#             reconstructed = torch.stack(reconstructed).to(self.device)
#         else:
...
#             correct += torch.sum(torch.abs(o - r) < threshold).item()
#             total += o.numel()
    ...
    def train(self):
        ...
        for epoch in epoch_it:
            ...
            for carrier, carrier_phase, msg in it:
                ...
                # Ensure avg_msg_loss is logged
                if 'avg_msg_loss' not in losses_log and 'msg_loss' in losses_log:
                    losses_log['avg_msg_loss'] = losses_log['msg_loss']
    ...
    def test(self, data='test'):
        ...
        with torch.no_grad():
            ...
            for carrier, carrier_phase, msg, msg_phase in tqdm(data):
                ...
                carrier_reconst, msg_reconst = self.forward(carrier, carrier_phase, msg)
                loss, losses_log = self.incur_loss(carrier, carrier_reconst, msg, msg_reconst)
                avg_carrier_loss += losses_log.get('carrier_loss', 0)
                avg_msg_loss += losses_log.get('avg_msg_loss', 0)  # Ensure the key matches
